...
public class MakeshiftLRDItem extends Item implements ISimpleMedicalUsable, IAllowInMedicbags, INbtDrivenDurability {
    ...
    public boolean onMedicalUse(Limb limb, ServerPlayer source, ServerPlayer target, ItemStack stack, InteractionHand hand) {
        target.getCapability(PlayerHealthProvider.PLAYER_HEALTH_DATA).ifPresent(h->{
            ...
            h.setLimbDesinfected(limb,Math.max(h.getLimbDesinfected(limb),7500));
            h.setInternalBleeding(h.getInternalBleeding()*0.75f);

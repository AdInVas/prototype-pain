...
public class NaloxoneVialItem extends MedicalVial implements IAllowInMedicbags {
    ...
    public void onCraftedBy(ItemStack pStack, Level pLevel, Player pPlayer) {
        ...
        if (!pStack.hasTag()) {
            addFluid(pStack,100,MedicalFluids.NALOXONE);
        }
    }

    @Override
    public void setupDefaults(ItemStack stack) {
        addFluid(stack,100,MedicalFluids.NALOXONE);
    }

    @Override
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        appendOrigHoverText(pStack, pLevel, pTooltipComponents, pIsAdvanced);
        pTooltipComponents.add(Component.translatable("item.prototype_pain.naloxone_vial.discription").withStyle(ChatFormatting.GRAY));

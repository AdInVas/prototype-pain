...
public class ReactionVialItem extends MedicalVial implements IAllowInMedicbags {



    @Override
    public void setupDefaults(ItemStack stack) {
        addFluid(stack, 100, MedicalFluids.REACTION_LIQUID);
    }

    @Override
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        appendOrigHoverText(pStack, pLevel, pTooltipComponents, pIsAdvanced);
        pTooltipComponents.add(Component.translatable("item.prototype_pain.reaction_vial.discription").withStyle(ChatFormatting.GRAY));
